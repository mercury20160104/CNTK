...
BOOST_AUTO_TEST_CASE(Index_with_caching)
{
    ...
    auto f1 = FileWrapper::OpenOrDie(L"test.tmp", L"rb");

    auto index = TextInputIndexBuilder(f1).Build();
    ...
    FILE* dummy = nullptr;
    auto cachedIndex = TextInputIndexBuilder(FileWrapper(L"test.tmp", dummy)).Build();
...
BOOST_AUTO_TEST_CASE(Index_64MB_with_caching_check_perf)
{
    ...
    for (int i = 0; i < 10; i++) 
    {
        ...
        auto index = TextInputIndexBuilder(f1).SetChunkSize(g_1MB >> 4).Build();
        ...
        start = GetTickCount();
        FILE* dummy = nullptr;
        auto cachedIndex = TextInputIndexBuilder(FileWrapper(filename, dummy)).SetChunkSize(g_1MB >> 4).Build();
        end = GetTickCount();
...
BOOST_AUTO_TEST_CASE(Index_1GB_with_caching_check_perf)
{
    ...
    BOOST_REQUIRE_EQUAL(0, timeToBuildIndexFromFile);

    // Cache is written out asynchronously in a separate thread, 
    Sleep(5000);  // sleep for a few seconds to give enough time to finish writing.
    start = GetTickCount();
    FILE* dummy = nullptr;
    auto cachedIndex = TextInputIndexBuilder(FileWrapper(filename, dummy)).Build();
    end = GetTickCount();
    timeToBuildIndexFromCache += (end - start);
