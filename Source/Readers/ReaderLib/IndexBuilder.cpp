...
IndexBuilder::IndexBuilder(const FileWrapper& input, bool enableCaching)
    : m_input(input),
    m_corpus(nullptr),
...
void IndexBuilder::WriteIndexCacheAsync(shared_ptr<Index>& index) 
{
    ...
    thread([cacheFilename, index]()
    {
        ...
        if (isCacheEnabled) 
        {
            _wrename(temp.c_str(), cacheFilename.c_str());
        }
    }).detach();
}
...
/*virtual*/ void TextInputIndexBuilder::Populate(shared_ptr<Index>& index) /*override*/
{
    ...
    while(isspace(m_reader->Peek()) && m_reader->Pop()); // skip leading white spaces.
